...
function FOETool(){
    ...
    self.settings = {
        path: {
            resources: "https://foe.neoapps.fr/iface/getResources.php",
            texts: "https://foe.neoapps.fr/iface/getTexts.php",
            plugins: "https://foe.neoapps.fr/var/plugins"
        }
    };
                    ...
                    load: function() {
                        self.res.plugins.map(function(plugin) {
                            ...
                            document.head.appendChild(new self.HTML().compose({
                                name: "script", attributes: {src: `${self.settings.path.plugins}/${plugin.name}/${plugin.name}.js?${timestamp}`},
                                properties: {
                                    onload: function(evt) {
                                        self.plugins.loaded++;
    ...
    self.init = function() {
        ...
        self.intervals['init'] = setInterval(function() {
            ...
            if (
                ...
                //    - Close the windows
                //    - Set visible
                self.data.windows.width = self.windows().self().offsetWidth;
                self.data.windows.height = self.windows().self().offsetHeight;

                self.windows().setPosition(
                    window.innerWidth - self.data.windows.width - 15,
                    window.innerHeight - self.data.windows.height - 130,
                );
