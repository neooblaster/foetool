...
function FOETool(){
    ...
    self.init = function() {
        ...
        self.intervals['init'] = setInterval(function() {
            if (self.res && !self.plugins.started) {
                // Loading Plugin Libs
                self.res.plugins.map(function(plugin) {
                    self.plugins.registred++;

                    document.head.appendChild(new self.HTML().compose({
                        name: "script", attributes: {src: `https://foe.neoblaster.fr/var/plugins/${plugin.name}/${plugin.name}.js`},
                        properties: {
                            onload: function(evt) {
                                self.plugins.loaded++;
            ...
            if (
                ...
                self.plugins.names.forEach(function(plugin) {
                    if (self.plugins.plugins[plugin] && self.plugins.plugins[plugin].init) {
                        self.plugins.plugins[plugin].init();
                    }
                });
    ...
    self.windows = function() {
        return {
            ...
            selectOption: function(option) {
                ...
                self.windows().getContent().innerHTML = '';
